...
class ServerHelper {
  ...
  static addErrorHandlers(expressApp, errorHandlers) {
    ...
    const codes = _StatusCode.default.values().map(code => code.toLowerCase());

    console.log('codes', codes); // for each error handler
    ...
    errorHandlers.forEach(errorHandler => {
      ...
      if (_Validator.default.hasErrorHandlerMethod(Instance)) {
        ...
        console.log('functions', functions); // for each error code

        codes.forEach(code => {
          // check if the error handler has a function for the code
          if (functions.indexOf(code) > -1) {
            console.log('handler has', code);
            console.log('hanlder', errorHandler); // if it does add it to the express routes for that error code

            const expressFunction = (expressRequest, expressResponse, nextHandler) => {
              console.log('ran error handler'); // create the request and response instances

              const request = new _Request.default(expressRequest);
              const response = new _Response.default(expressResponse); // check if the current status code is the error handler status code

              console.log('check if', _StatusCode.default.valueOf(code), ' is ', response.getStatus());

              if (_StatusCode.default.valueOf(code) === response.getStatus()) {

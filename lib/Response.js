...
const _circleAssign = _interopRequireDefault(require('circle-assign'));
...
class OctoResponse {
  ...
  attachment(filePath = '') {
    if (filePath === '') this.response.attachment(); else this.response.attachment(filePath);
    return this;
  }
  /**
  ...
  download(filePath, filename = '', options = {}, callback = () => {}) {
    ...
    if (downloadName === '') downloadName = _path.default.basename(filePath);
  ...
  redirect(urlOrPath, status = 302) {
    if (status !== 302) this.response.redirect(status, urlOrPath); else this.response.redirect(urlOrPath);
    return this;
  }
  /**
  ...
  render(view, data = {}, callback = undefined) {
    ...
    const locals = (0, _circleAssign.default)(this.response.app.defaultRenderData, data);
    if (callback === undefined) this.response.render(view, locals); else this.response.render(view, locals, callback);
    return this;
  }
  /**

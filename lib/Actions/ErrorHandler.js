...
class OctoErrorHandler extends _Action.default {
  ...
  static isHandler(Instance = null) {
    ...
    return Instance !== undefined && Instance !== null && instance instanceof OctoErrorHandler && (0, _internals.hasProp)(Instance, 'error', 'number', true, true) && (0, _internals.hasProp)(Instance, 'method', 'string', true) && (0, _internals.hasProp)(Instance, 'path', 'string', true) && (0, _internals.hasFunction)(instance, 'handle', true);
  }
  /**
  ...
  static attach(expressApp, Instance) {
    const bind = (methods = _Method.default.values()) => {
      const getExpressCallback = pathHook => async (request, response, next) => {
        const context = new _Context.default(request, response, next, pathHook);
      ...
      if (Instance.path !== undefined) {
        if (Array.isArray(Instance.path)) {
          Instance.path.forEach(path => {
            expressApp.use(Instance.path, getExpressCallback(path));
          });
        } else {
          expressApp.use(Instance.path, getExpressCallback(Instance.path));
        }
      } else {
        expressApp.use(getExpressCallback(undefined));
      }
    };

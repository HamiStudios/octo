...
class Server {
  constructor(options) {
    ...
    this.options = (0, _circleAssign.default)(this.defaultServerOptions, options);
    this.routers = [];
    this.routes = [];
    this.middlewares = [];
    this.errorHandlers = []; // create an express app
  ...
  errorHandler(pathOrInstance, instance = null) {
    ...
    if (instance !== null && new Instance() instanceof _ErrorHandler.default) {
      Instance = instance;
    }

    if (new Instance() instanceof _ErrorHandler.default) {
  ...
  start(callback = () => {}) {
    ...
    _ServerHelper.default.addMiddlewares(this.expressApp, this.middlewares.filter(mw => mw.afterRoutes === true)); // add error handlers to the express app


    _ServerHelper.default.addErrorHandlers(this.expressApp, this.errorHandlers); // check whether ssl is enabled and the private key and certificate paths exist
    ...
    if (this.options.ssl.enabled && privateKeyPath !== null && certificatePath !== null) {
      ...
      const privateKey = _fs.default.readFileSync(privateKeyPath);

      const certificate = _fs.default.readFileSync(certificatePath); // start a https server
